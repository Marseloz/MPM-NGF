...
class MPM:
  def __init__(self, snapshot_interval=20, **kwargs):
    res = kwargs['res']
## -----------------------------------------------------------------------------
    self.Snapshots = kwargs.get('snapshots', False)
## -----------------------------------------------------------------------------
    ...
    try:
      ...
      sys.exit()
    # simulation can be resumed using tcb filse --------------------------------
    ...
    ## create dummy levelset using resolution ----------------------------------
  ...
  def update_levelset(self, t0, t1):
    ...
    # initialize (from levelset.cpp)
    levelset.initialize(t0, t1,
                        self.levelset_generator(t0).levelset, # t0 = t
                        self.levelset_generator(t1).levelset) # t1 = t + t_step
    self.c.set_levelset(levelset)

  ## set_levelset called from python input file --------------------------------
  def set_levelset(self, levelset, is_dynamic_levelset=False):
    ...
    ## static levelset
      ...
      # _ means the last value OR insignificant variable ("don't care")
  ...
  def step(self, step_t, camera=None):
    ...
    ## update levelset @ t and t+step_t ----------------------------------------
  ...
  def clear_output_directory(self):
    ...
    #---------------------------------------------------------------------------
    taichi.clear_directory_with_suffix(frames_dir, 'txt')
    #---------------------------------------------------------------------------
    # taichi.clear_directory_with_suffix(self.snapshot_directory, 'tcb')

  ## simulate (MAIN LOOP) ------------------------------------------------------
  def simulate(self, clear_output_directory=False, print_profile_info=True,
               frame_update=None, update_frequency=1):
    # do restart
  ...
  def simulate_with_energy(self, clear_output_directory=False,
               print_profile_info=False, frame_update=None, update_frequency=1):
    # do restart
